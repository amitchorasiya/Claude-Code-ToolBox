...
> ### 🤝 Agentic Teams — swarm dispatch + multi-agent planning & debate (1.0.17+; swarm dispatch in 1.0.24)
>
> The **🤝 Agentic Teams** tab turns the hub into a **multi-agent planning &amp; debate** workbench on top of Claude Code's native subagent format. Specialised agents (product-manager, architect, security-reviewer, backend/frontend/qa/reviewer) **debate a design**, produce a **plan you approve**, then **execute** it — with a live color-coded transcript, token + cost tracking, and persisted `plan.md` / `decision.md` per run. Every team **is** a slash command — creating or editing a team auto-generates a `/command` that dispatches all agents **in parallel** via the Task tool (swarm pattern). 7 preset teams ship with the starter pack. Opt-in **Agent Dashboard** shows live cards for every Claude Code session on the machine. Cross-platform on macOS, Windows, and Linux. Full **IntelliJ parity** ships in the JetBrains plugin.
...
| **🤝 Agentic Teams** | **Multi-agent planning &amp; debate.** Agent CRUD (`~/.claude/agents/*.md`), team composition JSON, 8 protocols (incl. **debate + judge**, **plan-then-code with your approval gate**, and **converge** — parallel thinking → cross-pollination → synthesis), **swarm dispatch** (every team auto-generates a `/command` dispatching all agents in parallel), live transcript, **Agent Dashboard** opt-in card strip for every Claude Code session, and 7 preset swarm slash commands (`/debate-team`, `/plan-team`, `/review-team`, `/security-team`, `/sdlc-plan-then-code`, `/refactor-team`, `/spec-team`). |
...
- **Agent Teams:** 9-agent SDLC starter pack + 7 preset teams (`debate-team`, `plan-team`, `review-team`, `security-team`, `sdlc-plan-then-code`, `refactor-team`, `spec-team`) with swarm slash commands. 8 collaboration protocols including **debate + judge** (writes `decision.md`), **plan-then-code** (writes `plan.md`, blocks on your approval, then runs the code phase), and **converge** (parallel independent thinking → cross-pollination rounds → judge synthesis → approval → code execution). All JSONL-transcripted.
